...
\[
    m[i,j] = \begin{cases}
        ...
        w[i,j] + m[i+1,j] & \text{if } i < m \text{ and } j == n \\
        w[i,j] + m[i,j+1] & \text{if } i == m \text{ and } j < n \\
        w[i,j] + \min(m[i+1,j], m[i,j+1], m[i+1,j+1]) & \text {if } i < m \text{ and } j < n
    \end{cases}
\]
...
We start by initializing a 2D array cost[][] to be match the weight matrix.
We then iterate from the bottom right corner (m,n) to the top left corner (0,0), and update cost[row][col] to be the cost[row][col] + min(cost[row+1][col], cost[row][col+1], cost[row+1][col+1]).
The next array is also updated to the cell that provided the minimum cost.
